...
# part that builts a rootfs and copies it for the intiramfs
make clean
make BR2_EXTERNAL=$PWD/../Ajit_buildroot_configs Ajit_rootfs_defconfig
make && \
cp ./output/images/rootfs.tar ./../Ajit_buildroot_configs/board/overlay/initramfs

# make clean
make BR2_EXTERNAL=$PWD/../Ajit_buildroot_configs Ajit_initramfs_defconfig
make && \
cd -
